...
check () {
    ...
    if [ "$doit" == "1" ] && [ "$SKIPF16" == "1" ] && echo src/*/*/$FUNCTION.c | grep -q binary16; then
        echo "With SKIPF16, skipping " $FUNCTION
        doit=0
    fi
